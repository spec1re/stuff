...
# fix missing xmlrpc.inc
fetch -q -o /etc/inc/xmlrpc.inc https://raw.githubusercontent.com/pfsense/pfsense/RELENG_2_3_5/src/etc/inc/xmlrpc.inc
...
sed -i "" -E "s/(FreeBSD.*enabled:) no/\1 yes/" /usr/local/share/pfSense/pkg/repos/pfSense-repo.conf
sed -i "" -E "s/(FreeBSD.*enabled:) no/\1 yes/" /usr/local/etc/pkg/repos/FreeBSD.conf
...
if [ ! -f /usr/local/lib/libmilter.so.5 ];then
  ln -s /usr/local/lib/libmilter.so.7 /usr/local/lib/libmilter.so.5
fi
